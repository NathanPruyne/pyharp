...
# NOTE: UPDATE THIS WHENEVER CREATING A NEW INDEX
index_clap = pinecone.Index(os.environ["PC_API_KEY"], host='clap-jazz-blues-1-2-24-index-af8053a.svc.us-west1-gcp.pinecone.io')
...
def process_fn(input_audio_path, embedding_type):
    """
    ...
    if embedding_type == "clmr":
        embeddings = clmr_pipe(input_audio_path).get()[0]
        if len(embeddings.shape) == 1:
            embeddings = [embeddings]
        index = index_clmr
    elif embedding_type == "clap":
        process_sig = sig.copy().resample(48000).to_mono()
        process_sig = process_sig.zero_pad_to((process_sig.audio_data.shape[2] // 144000 + 1) * 144000).audio_data.squeeze().reshape((-1, 144000)).numpy()
        embeddings = clap_model.get_audio_embedding_from_data(process_sig)
        index = index_clap
    else:
        raise ValueError("Invalid type of embedding, valid types are \"clmr\" and \"clap\"")
